...
- Run `tools/check-daily-exists.sh` to check if today's note exists at `raw/daily/YYYY/MM/YYYY-MM-DD.md` (substitute actual date)
- If it exists: note it and proceed to step 2
- If it does not exist:
  - Warn the user
  - Ask: create it now using `tools/create-daily-note.sh`, or skip this step and proceed to step 2?
  - If approved: run `tools/create-daily-note.sh` — creates the note at the correct nested path from `raw/templates/daily.md` with today's date filled in
...
### 3. today's agenda vs home.md delta
